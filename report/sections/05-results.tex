...
\item Small sample size: only refactored one test suite version of one
  library. Mocha's test suite may not be representative of other test
  suites.
\item Refactoring assertions about API secrets out of API tests can
  result in duplicated testing code. This might be avoided by using a
  more complex test design, but that could reducing the readability of
  test, making them less useful as specifications.
\end{itemize}